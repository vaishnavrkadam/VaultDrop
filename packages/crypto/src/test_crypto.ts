...
import { ShamirSSS } from './threshold.js';
...
async function runTests() {
  ...
  // Test 1: Random Bytes
  try {
    const bytes = CryptoProvider.getRandomBytes(16);
    if (bytes.length !== 16) throw new Error('Random bytes size mismatch');
  ...
  try {
    ...
    salt.fill(0xab);
    const key = await CryptoProvider.deriveKeyFromPassword('super-secret-password', salt);
    if (key.length !== 32) throw new Error('Derived key length mismatch');
  ...
  try {
    ...
    masterKey.fill(0x01);
    const childKey = await CryptoProvider.deriveHKDF(masterKey, 'vaultdrop/test-purpose');
    if (childKey.length !== 32) throw new Error('HKDF derived key length mismatch');
  ...
  try {
    const key = CryptoProvider.getRandomBytes(32);
    ...
    const encrypted = await CryptoProvider.encryptAES_GCM(plaintext, key, associatedData);
  ...
  try {
    const recipientKey = CryptoProvider.generateBoxKeyPair();
    const message = new TextEncoder().encode('Secret Content Encryption Key (CEK)');

    const envelope = CryptoProvider.encryptForRecipient(message, recipientKey.publicKey);
    const decrypted = CryptoProvider.decryptForRecipient(envelope, recipientKey.secretKey);
  ...
  try {
    ...
    const shares = await ShamirSSS.splitSecret(secret, threshold, totalShares);
    if (shares.length !== totalShares) throw new Error('Split count mismatch');

    // Combine using exactly threshold (shares 0, 2, 4)
    const subShares = [shares[0], shares[2], shares[4]];
    const combined = await ShamirSSS.combineShares(subShares);
    ...
    try {
      await ShamirSSS.combineShares([shares[0], shares[1]]);
    } catch (e) {
