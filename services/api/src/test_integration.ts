...
app.post('/v1/shares', async (req, res) => {
  try {
    ...
    await dbRun(
      ...
      [shareId, shareType, accessMode, ciphertext, nonce, tag, wrappedContentKey || null, salt || null, burnAfterReading, fileMeta || null, expiresAt, createdAt]
    );
...
app.get('/v1/shares/:id/config', async (req, res) => {
  ...
  if (!share || share.consumed_at) return res.status(404).send('Not found');
...
app.post('/v1/shares/:id/consume', async (req, res) => {
  const { id } = req.params;
  await dbRun(`UPDATE shares SET consumed_at = ?, ciphertext = '', nonce = '', tag = '' WHERE id = ?`, [Date.now(), id]);
  res.send('Consumed');
});
...
async function runIntegrationTests() {
  ...
  const server = app.listen(3002, async () => {
    ...
    try {
      ...
      if (!createRes.ok) throw new Error('Create Share failed');
      ...
      const config = await configRes.json() as { id: string; burnAfterReading: boolean };
      if (!config.burnAfterReading) throw new Error('Config burn_after_reading mismatch');
      ...
      if (!payloadRes.ok) throw new Error('Fetch Payload failed');
      const body = await payloadRes.json() as { ciphertext: string };
      if (body.ciphertext !== 'base64-encoded-ciphertext') throw new Error('Payload ciphertext mismatch');
      ...
      if (!consumeRes.ok) throw new Error('Consume Share failed');
      ...
      if (verifyRes.status !== 404) throw new Error('Burned share is still available!');
      ...
      const { id: id2 } = await createRes2.json() as { id: string };
      
      const revokeRes = await fetch(`http://localhost:3002/v1/shares/${id2}`, { method: 'DELETE' });
      if (!revokeRes.ok) throw new Error('Delete Share failed');
      
      const verifyRes2 = await fetch(`http://localhost:3002/v1/shares/${id2}`);
      if (verifyRes2.status !== 404) throw new Error('Revoked share is still available!');
